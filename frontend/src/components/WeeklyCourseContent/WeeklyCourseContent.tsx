...
export default function WeeklyCourseContent({ 
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      ...
      setCourseError(null);
      try {
        const response = await fetch(`http://65.0.106.97/api/student/course/${courseId}/weeks`);
        if (!response.ok) {
  ...
  const fetchWeekContent = async (weekNo: number) => {
    ...
    try {
      const endpoint = isAdmin 
        ? `http://65.0.106.97/api/admin/weekwise-content/course/${courseId}/week/${weekNo}`
        : `http://65.0.106.97/api/admin/weekwise-content/course/${courseId}/week/${weekNo}`;
  ...
  const handleContentUpdate = async (weekNo: number, updatedContent: WeekContent) => {
    ...
    try {
      const response = await fetch(
        `http://65.0.106.97/api/admin/weekwise-content/${courseId}/${weekNo}`,
        {
          method: 'PUT',
          headers: {
